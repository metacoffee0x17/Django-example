...
class Article(TranslatedAutoSlugifyMixin,
    ...
    translations = TranslatedFields(
        ...
        additional_info=models.CharField(
            max_length=255, verbose_name=_('additional info'),
            blank=True, default=''),
        ...
        related_tag = models.ForeignKey(
            ...
            null=True,
            blank=True,
            verbose_name=_('related_tag'),
            on_delete=models.CASCADE,
        ),
        canonical_url=models.CharField(
            max_length=255, verbose_name=_('canonical url'),
            blank=True, default=''),
        no_amp = models.BooleanField(verbose_name=_('no amp'),
            blank=True, default='')
    )
    ...
    def get_article_tags(self, language=None):
        if not self.pk:
            return ''

        if language is None:
            language = get_current_language()
        return [tag.name.lower() for tag in self.tags.all()]
...
class NewsBlogLatestArticlesPlugin(PluginEditModeMixin,
    ...
    def get_articles(self, request):
        ...
        """
        # queryset = Article.objects
        queryset = Article.objects.all().filter(is_featured=True)
        ...
        try:
            ...
            page_tag = Page_Tag.objects.get(page_id=page_ids[0]['id']).page_tag
        except:
            return queryset[:self.latest_articles]
        result = [x for x in queryset if page_tag.lower() in x.get_article_tags()]
        return result[:self.latest_articles]
