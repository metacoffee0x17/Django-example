...
class CoffeeVideoPluginModel(CMSPlugin):
    ...
    def get_tags(self, request):
        ...
        query = """
            SELECT * FROM coffee_video_coffeevideo GROUP BY tag"""
        raw_tags = list(CoffeeVideo.objects.raw(query))
        tag_list = []
        for x in raw_tags:
            tags = x.tag.split(',')
            tag_list += tags
